...
if($action == 'clicks')
{
	...
	$q = 'SELECT * FROM links WHERE campaign_id = '.$campaign_id;
...
else if($action == 'opens')
{
	...
	$q = 'SELECT * FROM campaigns WHERE id = '.$campaign_id;
	$r = mysqli_query($mysqli, $q);
	if ($r && mysqli_num_rows($r) > 0)
	{
	    while($row = mysqli_fetch_array($r))
...
else if($action == 'unsubscribes')
{
	...
	$q = 'SELECT * FROM subscribers WHERE last_campaign = '.$campaign_id.' AND unsubscribed = 1';
	$r = mysqli_query($mysqli, $q);
	if ($r && mysqli_num_rows($r) > 0)
	{
...
else if($action == 'bounces')
{
	...
	$q = 'SELECT * FROM subscribers WHERE last_campaign = '.$campaign_id.' AND bounced = 1';
	$r = mysqli_query($mysqli, $q);
	if ($r && mysqli_num_rows($r) > 0)
	{
		$unsubscribes_array = array();
...
else if($action == 'complaints')
{
	...
	$q = 'SELECT * FROM subscribers WHERE last_campaign = '.$campaign_id.' AND complaint = 1';
	$r = mysqli_query($mysqli, $q);
	if ($r && mysqli_num_rows($r) > 0)
	{
		$unsubscribes_array = array();
