...
//get user details
$q = 'SELECT name, username FROM login WHERE id = '.get_app_info('main_userID');
...
$q = 'SELECT from_name, from_email, reply_to, title, plain_text, html_text, to_send FROM campaigns WHERE id = '.$campaign_id.' AND userID = '.get_app_info('main_userID');
$r = mysqli_query($mysqli, $q);
if ($r && mysqli_num_rows($r) > 0)
{
    while($row = mysqli_fetch_array($r))
...
if ($r && mysqli_num_rows($r) > 0)
{
    ...
    while($row = mysqli_fetch_array($r))
    {
		...
		{			
		    while($row2 = mysqli_fetch_array($r2))
		    {
				...
				//replace new links on HTML code
		    	$html_treated = str_replace('href="'.$link.'"', 'href="'.get_app_info('path').'/l/'.short($subscriber_id).'/'.short($linkID).'"', $html_treated);
		    	$html_treated = str_replace('href=\''.$link.'\'', 'href="'.get_app_info('path').'/l/'.short($subscriber_id).'/'.short($linkID).'"', $html_treated);
		    	
		    	//replace new links on Plain Text code
		    	$plain_treated = str_replace($link, get_app_info('path').'/l/'.short($subscriber_id).'/'.short($linkID), $plain_treated);
		    }  
		}
		...
		if(get_app_info('s3_key')!='' && get_app_info('s3_secret')!='')
		{
			...
			//otherwise send with curl_multi
				...
				$mail->IsAmazonSES(true, $campaign_id, $subscriber_id, get_app_info('timezone'));
		...
		//if sent to all recipients, break out of loop (just in case, even if loop will end)
    ...
    if ($r4 && mysqli_num_rows($r4) > 0)
    {
        while($row = mysqli_fetch_array($r4))
        {
	        ...
	        {
	        	//increment recipients number in campaigns table
				$q6 = 'UPDATE campaigns SET recipients = recipients+1 WHERE id = '.$campaign_id;
				mysqli_query($mysqli, $q6);
...
"._('View report')." - $app_path/report?i=$app&c=$campaign_id";
	...
	if($no_of_recipients >= $to_send) $mail2->Send();	
}
