...
<?php 	
	//setup cron
	$q = 'SELECT id, cron FROM login LIMIT 1';
				    ...
				    while($row = mysqli_fetch_array($r))
				    {
						...
						{			
						    while($row2 = mysqli_fetch_array($r2))
						    {
								...
								//replace new links on HTML code
						    	$html_treated = str_replace('href="'.$link.'"', 'href="'.APP_PATH.'/l/'.short($subscriber_id).'/'.short($linkID).'"', $html_treated);
						    	$html_treated = str_replace('href=\''.$link.'\'', 'href="'.APP_PATH.'/l/'.short($subscriber_id).'/'.short($linkID).'"', $html_treated);
						    	
						    	//replace new links on Plain Text code
						    	$plain_treated = str_replace($link, APP_PATH.'/l/'.short($subscriber_id).'/'.short($linkID), $plain_treated);
						    }  
						}
						...
						//if sent to all recipients, come out of loop (just in case, even if loop will end)
