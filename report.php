...
<?php 
	if(get_app_info('is_sub_user')) 
	{
		...
		$q = 'SELECT app FROM campaigns WHERE id = '.mysqli_real_escape_string($mysqli, $_GET['c']);
...
?>
<?php 
	$q = 'SELECT * FROM campaigns WHERE userID = '.get_app_info('main_userID').' AND app='.get_app_info('app').' AND id = '.mysqli_real_escape_string($mysqli, $_GET['c']);
	$r = mysqli_query($mysqli, $q);
	if ($r && mysqli_num_rows($r) > 0)
	{
	    while($row = mysqli_fetch_array($r))
	    {
			$id = stripslashes($row['id']);
	  		...
	  		$click_per = round(get_click_percentage($_GET['c'])/($recipients-get_bounced()) *100, 4);
    ...
    $(document).ready(function() {
        ...
        chart = new Highcharts.Chart({
            ...
            },
            {
                ...
                data: [<?php echo get_click_percentage($_GET['c']);?>]
            },
            {
...
<div class="row-fluid">
    ...
    <div class="span10">
    	...
    	<h3><?php echo get_saved_data('title');?> <a href="<?php echo get_app_info('path');?>/w/<?php echo short($id);?>" title="<?php echo _('View the campaign');?>" class="iframe-preview"><span class="icon-eye-open"></span></a></h3><br/>
    	...
    	<div class="row-fluid">
	    	<div class="span6">
	    		<div class="well">
			    	...
			    	<h3><span class="badge badge-info" style="font-size:16px;"><?php echo $click_per;?>%</span> <?php echo _('clicked a link');?> <span class="label"><?php echo get_click_percentage($_GET['c']);?> <?php echo _('clicked');?></span></h3>
			    </div>
	    	</div>
	    ...
	    <div class="row-fluid">
	    	<div class="span12">
		    	...
		    	<a href="<?php echo get_app_info('path');?>/includes/reports/export-csv.php?c=<?php echo $id?>&a=clicks" title="<?php echo _('Export subscribers who clicked');?>" class="report-export"><i class="icon icon-download-alt"></i></a>
	    	</div>
	    </div>
	    <br/>
	    <div class="row-fluid">
	    	<table class="table table-striped table-condensed responsive">
			  ...
			  <tbody>
			  	...
			  	<?php 
				  	$q = 'SELECT id, link, clicks FROM links WHERE campaign_id = '.mysqli_real_escape_string($mysqli, $_GET['c']);
				  	$r = mysqli_query($mysqli, $q);
				  	if ($r && mysqli_num_rows($r) > 0)
				  	{
				  	    while($row = mysqli_fetch_array($r))
				  	    {
						      ...
						      <td>'.$unique_clicks.'</td>
						      <td>'.$total_clicks.'</td>
						      <td><a href="'.get_app_info('path').'/includes/reports/export-csv.php?c='.$id.'&l='.$link_id.'&a=recipient_clicks" title="'._('Export subscribers who clicked this link').'" class="recipient-click-export"><i class="icon icon-download-alt"></i></a></td>
						    </tr>				  			
				  			';
				  	    }  
				  	}
	    ...
	    <div class="row-fluid">
	    	<div class="span12">
		    	...
		    	<a href="<?php echo get_app_info('path');?>/includes/reports/export-csv.php?c=<?php echo $id?>&a=opens" title="<?php echo _('Export subscribers who opened');?>" class="report-export"><i class="icon icon-download-alt"></i></a>
	    	</div>
	    </div>
	    <br/>
	    <div class="row-fluid">
	    	<table class="table table-striped table-condensed responsive">
			  ...
			  <tbody>
			  	...
			  	<?php 
				  	$q = 'SELECT opens from campaigns WHERE id = '.mysqli_real_escape_string($mysqli, $_GET['c']);
				  	$r = mysqli_query($mysqli, $q);
				  	if ($r && mysqli_num_rows($r) > 0)
				  	{
				  	    while($row = mysqli_fetch_array($r))
	    ...
	    <div class="row-fluid">
	    	<div class="span12">
		    	...
		    	<a href="<?php echo get_app_info('path');?>/includes/reports/export-csv.php?c=<?php echo $id?>&a=unsubscribes" title="<?php echo _('Export subscribers who unsubscribed');?>" class="report-export"><i class="icon icon-download-alt"></i></a>
	    	</div>
	    </div>
	    <br/>
	    <div class="row-fluid">
	    	<table class="table table-striped table-condensed responsive">
			  ...
			  <tbody>
			  	...
			  	<?php 
				  	$q = 'SELECT * FROM subscribers WHERE unsubscribed = 1 AND last_campaign = '.mysqli_real_escape_string($mysqli, $_GET['c']).' ORDER BY timestamp DESC LIMIT 10';
				  	$r = mysqli_query($mysqli, $q);
				  	if ($r && mysqli_num_rows($r) > 0)
				  	{
				  	    while($row = mysqli_fetch_array($r))
	    ...
	    <div class="row-fluid">
	    	<div class="span12">
		    	...
		    	<a href="<?php echo get_app_info('path');?>/includes/reports/export-csv.php?c=<?php echo $id?>&a=bounces" title="<?php echo _('Export subscribers who bounced');?>" class="report-export"><i class="icon icon-download-alt"></i></a>
	    	</div>
	    </div>
	    <br/>
	    <div class="row-fluid">
	    	<table class="table table-striped table-condensed responsive">
			  ...
			  <tbody>
			  	...
			  	<?php 
				  	$q = 'SELECT * FROM subscribers WHERE bounced = 1 AND last_campaign = '.mysqli_real_escape_string($mysqli, $_GET['c']).' ORDER BY timestamp DESC LIMIT 10';
				  	$r = mysqli_query($mysqli, $q);
				  	if ($r && mysqli_num_rows($r) > 0)
				  	{
				  	    while($row = mysqli_fetch_array($r))
	    ...
	    <div class="row-fluid">
	    	<div class="span12">
		    	...
		    	<a href="<?php echo get_app_info('path');?>/includes/reports/export-csv.php?c=<?php echo $id?>&a=complaints" title="<?php echo _('Export subscribers who marked your email as spam');?>" class="report-export"><i class="icon icon-download-alt"></i></a>
	    	</div>
	    </div>
	    <br/>
	    <div class="row-fluid">
							...
							else
							{
					  			foreach($country_count_array as $cca)
					  			{
						  			...
						  			<tr>
						  				<td>'.$cc[1].'</td>
						  				<td>'.$cc[0].'</td>
						  				<td><a href="'.get_app_info('path').'/includes/reports/export-csv.php?c='.$id.'&a='.$cc[2].'" title="'._('Export subscribers from').' '.$cc[1].'"><i class="icon icon-download-alt"></i></a></td>
						  			</tr>
						  			';
					  			}
